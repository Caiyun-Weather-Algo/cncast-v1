...
def runner(args):
    ...
    logger.log(f"ifs dataloader done, dataloader length {len(dataset)}!")
    ...
    for k, data in enumerate(loader):
        if args.data_source=="ifs":
            ''' real-time inference for online data '''
            sample_start_time, xin = data
        else:
            sample_start_time, xin, _, _ = data
            logger.log(f"data prepared for {current_tstr}, start iterative prediction")
        
        ''' make directories '''
        current_time = int(sample_start_time.detach().numpy())
        current_tstr = arrow.get(int(sample_start_time.detach().numpy())).format("YYYYMMDDHH")
        tdir = arrow.get(int(sample_start_time.detach().numpy())).format("YYYY/MM/DD/HH")
        ...
        t0 = time.time()
        fcst_surf, fcst_high = iter_predict(xin, current_time, cfg, static_args)
        ...
        if args.data_format=="zarr":
            surf = util.to_dataset(surf_fc, fcst_ts, cfg.input.surface, lats, lons, levels=None)
            high = util.to_dataset(high_fc, fcst_ts, cfg.input.high, lats, lons, levels=levels)
        ...
        if args.data_format=="zarr":
            ds_tp.to_zarr(f2save_grib.replace(".grib2", ".zarr"), consolidated=True)
        elif args.data_format=="grib2":
        ...
        if cfg.hyper_params.tp_high_res:
            ...
            if args.data_format=="zarr":
                ds_tp_sr.to_zarr(f2save_grib.replace(".grib2", ".zarr"), consolidated=True)
            else:
...
def main():
    ...
    parser.add_argument("--forecast_day", type=int, default=1, help="max lead time of days")
    parser.add_argument("--log_dir", type=str, default="./logs", help="log directory")
    parser.add_argument("--data_source", type=str, default="era5", help="data source, ifs or era5")
    parser.add_argument("--data_format", type=str, default="zarr", help="format of saving data, grib2 or zarr")
